# ==============================================================================
# EXERCISM TCL PRACTICE: PIECING-IT-TOGETHER
# ==============================================================================
# # Instructions
#
...
proc checkJigsaw {input rows columns resultsName} {
    ...
    # Bỏ qua kích thước không khớp dữ liệu
...
proc jigsawData {input} {
    ...
    if {[dict exists $input pieces]} {
        ...
        for {set rows 1} {$rows <= $pieces} {incr rows} {
            if {$pieces % $rows == 0} {
                checkJigsaw $input $rows [expr {$pieces / $rows}] candidates
            }
